...
image_save_dir = "/home/fizzer/ros_ws/src/Track_images_V2"
os.makedirs(image_save_dir, exist_ok=True)

folder_id = "1Kmc7bQYfkbydX6IN2bSwjRGKK7ajUeEC"  # Replace this with your actual Drive folder ID
...
# Commented so that it doesnt crash  
...
def save_image_with_one_hot_encoding(one_hot_encoding):
    ...
    # Extract and format linear and angular values
    linear = float(one_hot_encoding[0])
    angular = float(one_hot_encoding[1])
    linear_str = f"{linear:.2f}"
    angular_str = f"{angular:.2f}"

    filename = os.path.join(image_save_dir, f"{timestamp:.6f}_lin{linear_str}_ang{angular_str}.jpg")
...
def cmd_vel_callback(msg):
    ...
    if not recording_active or rospy.is_shutdown():
        return
    current_encoding = [msg.linear.x, msg.angular.z]
